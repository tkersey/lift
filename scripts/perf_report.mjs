#!/usr/bin/env node
// Zig-first launcher for lift perf_report with Homebrew tap bootstrap and Python fallback.
...
const zigBinary = resolve(here, "perf_report_zig");
const zigSource = resolve(here, "perf_report.zig");
const pyFallback = resolve(here, "perf_report.py");
const tapFormula = process.env.LIFT_PERF_REPORT_FORMULA ?? "tkersey/tap/lift-perf-report";
const tapBinary = process.env.LIFT_PERF_REPORT_BIN ?? "lift-perf-report";
